export function sentenceCase(s){
  if(!s) return s
  s = s.trim()
  // birden fazla cümle varsa her birini düzelt
...
export function tidy(s){
  if(!s) return s
  // normalize unicode spaces, multiple spaces, fix spacing before punctuation etc.
  s = s.replace(/\s+/g,' ').replace(/\s+([,.;:!?])/g,'$1').trim()
  // ensure punctuation at end if it's a short sentence
